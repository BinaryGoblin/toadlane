  ...
  def update_fly_buy_profile
    if request.post? && fly_buy_params.present?
      ...
      client_user = FlyBuyService.get_user(
                            oauth_key: nil, fingerprint: fly_buy_profile.encrypted_fingerprint, 
      ...
      update_user_response = client_user.users.update(payload: update_payload)
      ...
      add_documents_payload = {
        'physical_docs' => [{
          ...
          'document_type' => FlyAndBuy::AddingBankDetails::SynapsePayDocType[:ein]
        }]
      }
