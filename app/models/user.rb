...
class User < ActiveRecord::Base
  ...
  def stripe_profile_verified?
    self.stripe_profile.present?
  end

  def green_profile_verified?
    self.green_profile.present?
  end

  def amg_profile_verified?
    self.amg_profile.present?
  end

  def emb_profile_verified?
    self.emb_profile.present?
  end
  ...
  def related_products
    Product.joins('LEFT JOIN fly_buy_orders ON fly_buy_orders.product_id = products.id')
      ...
      .where('(fly_buy_orders.buyer_id = ?
        ...
        AND products.end_date >= ?
        AND products.status = ?',
        id, id, id, id, id, id, id, Time.now, Time.now, true)
      .uniq
  end
